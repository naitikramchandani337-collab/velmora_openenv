...
# lazy import so startup never crashes
def get_env_classes():
    ...
    return IncidentEnv, Action, grade_task
...
@app.post("/reset")
def reset(task: str = "easy", body: Optional[ResetRequest] = None):
    ...
    try:
        IncidentEnv, _, __ = get_env_classes()
...
def grader(session_id: str, task: str = "easy"):
    ...
    if env is None:
        raise HTTPException(status_code=400, detail="Invalid session_id. Call /reset first.")
    try:
        _, __, grade_task = get_env_classes()
