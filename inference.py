...
def system_prompt() -> str:
    return (
        ...
        "Rules:\n"
        "- LOW severity: go straight to 'fix' for obvious issues (typo, broken link, UI). "
        "Only 'investigate' if cause is unclear.\n"
        "- MEDIUM severity: start with 'investigate', only 'escalate' if logs mention "
        "external/vendor/gateway, then 'fix', then 'monitor'.\n"
        "- HIGH severity: start with 'contain' for security/breach, then 'investigate', "
        "then 'escalate', then 'fix', then 'monitor'.\n"
...
def parse_action(raw_text: str) -> str:
    ...
    for action in VALID_ACTIONS:
        if action in text:
            return action
    print(f"[WARN] Could not parse action from: {raw_text!r}, defaulting to 'investigate'", flush=True)
    return "investigate"
...
def run_single_task(task: str, client: OpenAI, model_name: str) -> Dict[str, Any]:
    ...
    try:
        while not done and step_idx < MAX_STEPS:
            ...
            obs, reward, done, info = env.step(Action(action=action_text))
            step_idx += 1
...
def main():
    ...
    for task in TASKS:
        try:
            ...
            emit("[END]", {"success": "false", "steps": 0, "score": "0.00", "rewards": ""})

    print(
        f"[SUMMARY] tasks={len(all_results)} runtime={round(time.time() - started_at, 2)}s",
        flush=True,
    )
